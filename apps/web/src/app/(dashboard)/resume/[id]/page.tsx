...
import { useResumeStore, ResumeState, TemplateType, SkillCategory, Resume } from "@/stores/resume-store";
...
// Re-use step components from builder
// These are inlined here for the edit page context
...
const STEPS: Step[] = [
  ...
  { id: 1, label: "Experience", icon: <Briefcase className="w-4 h-4" /> },
  { id: 2, label: "Education", icon: <GraduationCap className="w-4 h-4" /> },
  { id: 3, label: "Skills", icon: <Star className="w-4 h-4" /> },
  { id: 4, label: "Additional", icon: <Plus className="w-4 h-4" /> },
  { id: 5, label: "ATS Score", icon: <Zap className="w-4 h-4" /> },
  { id: 6, label: "Preview", icon: <Eye className="w-4 h-4" /> },
  { id: 7, label: "Save", icon: <Save className="w-4 h-4" /> },
];
...
function useAutoSaveEdit(store: ResumeState, resumeId: string) {
  ...
  useEffect(() => {
    timerRef.current = setInterval(() => { if (isDirtyRef.current) save(); }, 30000);
...
function StepProfile({ store }: { store: ResumeState }) {
  ...
  const handleGenerateSummary = async () => {
    setGenerating(true);
    try {
      const res = await resumeApi.generateSummary({ prompt: "Generate a professional summary", currentSummary: summary });
      const generated = res.data?.data ?? res.data?.summary ?? "";
      if (generated) { updateSummary(generated); toast.success("Summary generated!"); }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {fields.map(f => (
          <div key={f.key} className="space-y-1.5">
            <Label htmlFor={`edit-${f.key}`}>{f.label} {f.required && <span className="text-error">*</span>}</Label>
            <Input id={`edit-${f.key}`} type={f.type ?? "text"} placeholder={f.placeholder}
              value={(header as any)[f.key] ?? ""} onChange={(e) => updateHeader(f.key, e.target.value)} />
          </div>
        ))}
      </div>
      ...
      <div className="space-y-2">
        ...
        <Input id="edit-title" placeholder="e.g., Senior Frontend Engineer"
          value={resume?.title ?? ""} onChange={(e) => store.setResumeTitle(e.target.value)} />
      </div>
      ...
      <div className="space-y-2">
        ...
        <Textarea id="edit-summary" value={summary} onChange={(e) => updateSummary(e.target.value)}
          placeholder="Write a brief summary of your experience, skills, and what you're looking for..."
          className="min-h-[120px]" maxLength={600} />
      </div>
    </div>
  );
}
...
function StepExperience({ store }: { store: ResumeState }) {
  ...
  const updateExp = (id: string, field: string, value: any) => {
    if (!resume) return;
    store.updateSection("experience", resume.experience.map(e => e.id === id ? { ...e, [field]: value } : e));
  };
  ...
  const handleSuggest = async (id: string) => {
    const exp = resume?.experience.find(e => e.id === id);
    if (!exp?.title || !exp?.company) { toast.error("Fill in job title and company first"); return; }
    try {
      const res = await resumeApi.suggestAchievements({ jobTitle: exp.title, company: exp.company, bullets: exp.bullets });
      const suggested: string[] = res.data?.data ?? res.data?.achievements ?? [];
      if (suggested.length > 0) { updateExp(id, "bullets", [...exp.bullets, ...suggested]); toast.success(`Added ${suggested.length} suggestions`); }
    } catch { toast.error("Failed to suggest achievements"); }
  };
            ...
            {expanded === exp.id && (
              <div className="px-4 pb-4 space-y-4 border-t border-outline-variant pt-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1"><Label>Job Title *</Label><Input value={exp.title} onChange={(e) => updateExp(exp.id, "title", e.target.value)} placeholder="Software Engineer" /></div>
                  <div className="space-y-1"><Label>Company *</Label><Input value={exp.company} onChange={(e) => updateExp(exp.id, "company", e.target.value)} placeholder="Acme Corp" /></div>
                  <div className="space-y-1"><Label>Location</Label><Input value={exp.location ?? ""} onChange={(e) => updateExp(exp.id, "location", e.target.value)} placeholder="San Francisco, CA" /></div>
                  <div className="space-y-1"><Label>Employment Type</Label>
                    <Select value={exp.employmentType ?? "Full-time"} onValueChange={(v) => updateExp(exp.id, "employmentType", v)}>
                  ...
                  <div className="space-y-1"><Label>Start Date</Label><Input type="month" value={exp.startDate} onChange={(e) => updateExp(exp.id, "startDate", e.target.value)} /></div>
                  <div className="space-y-1"><Label>End Date</Label><Input type="month" value={exp.endDate} onChange={(e) => updateExp(exp.id, "endDate", e.target.value)} /></div>
                </div>
                <div className="flex items-center gap-2">
                  <Checkbox id={`current-${exp.id}`} checked={exp.isCurrent} onCheckedChange={(checked) => { updateExp(exp.id, "isCurrent", !!checked); if (checked) updateExp(exp.id, "endDate", ""); }} />
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => handleSuggest(exp.id)}>
                  ...
                  {exp.bullets.map((bullet, bi) => (
                    <div key={bi} className="flex gap-2 items-start">
                      ...
                      <Input value={bullet} onChange={(e) => updateBullet(exp.id, bi, e.target.value)}
...
function StepEducation({ store }: { store: ResumeState }) {
  ...
  const addEducation = () => {
    store.addEducation({ school: "", degreeType: "", fieldOfStudy: "", graduationDate: "", gpa: "", honors: "", coursework: [], extracurriculars: "" });
  ...
  const updateEdu = (id: string, field: string, value: any) => {
    if (!resume) return;
    store.updateSection("education", resume.education.map(e => e.id === id ? { ...e, [field]: value } : e));
  };
              ...
              <div className="px-4 pb-4 space-y-3 border-t border-outline-variant pt-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1"><Label>Degree *</Label>
                    <Select value={edu.degreeType} onValueChange={(v) => updateEdu(edu.id, "degreeType", v)}>
                  ...
                  <div className="space-y-1"><Label>Field of Study *</Label><Input value={edu.fieldOfStudy} onChange={(e) => updateEdu(edu.id, "fieldOfStudy", e.target.value)} placeholder="Computer Science" /></div>
                  <div className="space-y-1"><Label>Institution *</Label><Input value={edu.school} onChange={(e) => updateEdu(edu.id, "school", e.target.value)} placeholder="MIT" /></div>
                  <div className="space-y-1"><Label>GPA (optional)</Label><Input value={edu.gpa ?? ""} onChange={(e) => updateEdu(edu.id, "gpa", e.target.value)} placeholder="3.8/4.0" /></div>
                  <div className="space-y-1"><Label>Graduation Date</Label><Input type="month" value={edu.graduationDate} onChange={(e) => updateEdu(edu.id, "graduationDate", e.target.value)} /></div>
                  <div className="space-y-1"><Label>Honors</Label><Input value={edu.honors ?? ""} onChange={(e) => updateEdu(edu.id, "honors", e.target.value)} placeholder="Magna Cum Laude" /></div>
                </div>
                <div className="space-y-1">
                  <Label>Relevant Coursework</Label>
                  <Input value={edu.coursework.join(", ")} onChange={(e) => updateEdu(edu.id, "coursework", e.target.value.split(",").map(s => s.trim()).filter(Boolean))}
                    placeholder="Data Structures, Algorithms (comma-separated)" />
                </div>
                <div className="flex justify-end">
                  <Button size="sm" variant="ghost" className="text-error" onClick={() => { store.removeEducation(edu.id); setExpanded(null); }}>
                    <Trash2 className="w-3 h-3 mr-1" /> Remove
                  </Button>
                </div>
              </div>
...
function StepSkills({ store }: { store: ResumeState }) {
  ...
  const handleAddSkill = (name: string, category: SkillCategory, proficiency: 1|2|3|4 = 2) => {
    if (!name.trim()) return;
    if (skills.some(s => s.name.toLowerCase() === name.toLowerCase().trim())) { toast.error("Skill already added"); return; }
  ...
  const handleMatch = async () => {
    ...
    try {
      ...
      suggested.forEach(s => {
        if (!skills.some(ex => ex.name.toLowerCase() === s.toLowerCase())) store.addSkill(s, activeCategory, 2);
      });
      if (suggested.length > 0) toast.success(`Added ${suggested.length} suggested skills`);
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        <Input value={skillInput} onChange={(e) => setSkillInput(e.target.value)}
          onKeyDown={(e) => { if (e.key === "Enter" || e.key === ",") { e.preventDefault(); handleAddSkill(skillInput, activeCategory); } }}
        ...
        <Button variant="outline" onClick={handleMatch}><Sparkles className="w-4 h-4 mr-1" /> Match</Button>
      </div>
...
function StepAdditional({ store }: { store: ResumeState }) {
  ...
  const renderCertifications = () => {
    ...
    const add = () => { store.addCertification({ name: "", issuer: "", date: "" }); };
  ...
  const renderProjects = () => {
    ...
    const add = () => { store.addProject({ name: "", description: "", technologies: [] }); };
        ...
        {projects.map(proj => (
          <Card key={proj.id} className="p-4 bg-surface-container-high shadow-ambient-sm space-y-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              ...
              <Input value={proj.technologies.join(", ")} onChange={(e) => update(proj.id, "technologies", e.target.value.split(",").map(s => s.trim()).filter(Boolean))} placeholder="React, Node.js" />
            </div>
            <Textarea value={proj.description} onChange={(e) => update(proj.id, "description", e.target.value)} placeholder="Brief description..." className="min-h-[80px]" />
            <div className="flex justify-end">
  ...
  const renderInterests = () => {
    ...
    const add = (name: string) => { if (!name.trim()) return; store.addInterest(name.trim()); setInput(""); };
    return (
      <div className="space-y-3">
        <div className="flex gap-2">
...
function StepATS({ store, resumeId }: { store: ResumeState; resumeId: string }) {
  const resume = store.currentResume;
  ...
  const handleScore = async () => {
    ...
    setScoring(true);
    try {
      const res = await resumeApi.getAtsScore({ resumeId, jobDescription });
  ...
  const handleAutoOptimize = async () => {
    ...
    setOptimizing(true);
    try {
      await resumeApi.autoOptimize({ resumeId });
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-on-surface-variant">Analyze and optimize your resume</p>
      </div>
      ...
      <div className="space-y-2">
        ...
        <Textarea id="ats-jd" value={jobDescription} onChange={(e) => setJobDescription(e.target.value)}
          placeholder="Paste the job description here..." className="min-h-[150px]" />
        <div className="flex gap-2">
...
function StepPreview({ store, resumeId }: { store: ResumeState; resumeId: string }) {
  const resume = store.currentResume;
  ...
  const handleDownload = async () => {
    ...
    try {
      const res = format === "pdf"
        ? await resumeApi.downloadPdfById(resumeId)
        : await resumeApi.downloadDocxById(resumeId);
  ...
  const data = resume ?? {
    name: "Untitled", title: "", template: "MODERN" as TemplateType,
      ...
      <div className="transform scale-[0.65] origin-top-left" style={{ width: "154%", height: "154%" }}>
        <div className="bg-white rounded-xl shadow-ambient-lg p-8 min-h-[700px] font-sans border border-outline-variant">
          <div className="text-center mb-4">
            <h1 className="text-2xl font-bold text-on-surface">{data.header.name || "Your Name"}</h1>
          ...
          {data.summary && (
            <div className="mb-4">
              ...
              <p className="text-xs text-on-surface-variant">{data.summary}</p>
            </div>
          )}
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-on-surface-variant">Review and save your changes</p>
      </div>
      ...
      <Card className="p-6 bg-surface-container-highest shadow-ambient-sm max-w-md mx-auto space-y-4">
        <div className="space-y-1.5">
          ...
          <Input id="save-name" value={name} onChange={(e) => setName(e.target.value)}
            placeholder="e.g., Senior Frontend Dev — TechCorp" />
        </div>
        <div className="space-y-1.5">
          <Label htmlFor="save-title">Target Role</Label>
          <Input id="save-title" value={title} onChange={(e) => setTitle(e.target.value)}
            placeholder="e.g., Senior Frontend Engineer" />
        </div>
        <div className="flex items-center gap-2">
          <Checkbox id="save-draft" checked={store.currentResume?.status === "draft"} onCheckedChange={(checked) => store.setStatus(checked ? "draft" : "complete")} />
...
export default function ResumeEditPage() {
  ...
  const { saveStatus } = useAutoSaveEdit(store, resumeId);
  ...
  const renderStep = () => {
    switch (currentStep) {
      case 0: return <StepProfile store={store} />;
      case 1: return <StepExperience store={store} />;
      case 2: return <StepEducation store={store} />;
      case 3: return <StepSkills store={store} />;
      case 4: return <StepAdditional store={store} />;
      case 5: return <StepATS store={store} resumeId={resumeId} />;
      case 6: return <StepPreview store={store} resumeId={resumeId} />;
      case 7: return <StepSave store={store} resumeId={resumeId} onComplete={handleComplete} />;
      default: return null;
    }
  };
        ...
        <div className="max-w-4xl mx-auto px-4 py-3">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-display text-lg font-bold text-on-surface">Edit Resume</h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <Button size="sm" variant="outline" onClick={() => router.push("/resume")}>Exit</Button>
            </div>
          </div>
