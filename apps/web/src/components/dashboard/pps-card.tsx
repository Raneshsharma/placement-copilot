...
export function PPSCard({ score, breakdown, suggestions }: PPSCardProps) {
  ...
  const scoreColor = score >= 80 ? "#22C55E" : score >= 60 ? "#0D7377" : "#F59E0B";
        ...
        <div className="flex flex-col items-center lg:items-start">
          ...
          <div className="relative w-36 h-36">
            <svg className="w-36 h-36 -rotate-90" viewBox="0 0 120 120">
              <circle cx="60" cy="60" r="54" fill="none" stroke="#E8E8E6" strokeWidth="8" />
              <circle
                cx="60"
                cy="60"
                r="54"
  ...
  return (
    <div className="space-y-3">
      {applications.slice(0, 3).map((app) => (
        <div key={app.id} className="flex items-center gap-3 p-3 rounded-lg bg-surfaceAlt">
          ...
          <Badge variant={app.status === "INTERVIEW" ? "success" : app.status === "UNDER_REVIEW" ? "yellow" : "gray"}>
  ...
  return (
    <div className="space-y-3">
      {interviews.map((interview) => (
        <div key={interview.id} className="flex items-center gap-3 p-3 rounded-lg bg-surfaceAlt">
          ...
          <Badge variant="coral">{interview.type}</Badge>
        </div>
      ))}
    </div>
  );
...
export function RoleRecommendations({ roles }: RoleRecommendationsProps) {
  return (
    <div className="flex gap-4 overflow-x-auto pb-2">
      {roles.map((role) => (
        <div key={role.id} className="flex-shrink-0 w-64 p-4 rounded-lg border border-border bg-surface hover:shadow-md transition-shadow">
          <div className="flex items-center gap-3 mb-3">
            ...
            <Badge variant={role.match >= 80 ? "success" : role.match >= 60 ? "default" : "gray"}>
              {role.match}%
            </Badge>
          </div>
