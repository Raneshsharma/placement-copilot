...
export class ProgressService {
  ...
  async getDashboard(userId: string) {
    const [applications, interviews, profile] = await Promise.all([
      this.prisma.application.findMany({ where: { userId }, include: { jobListing: true } }),
  ...
  async getAnalytics(userId: string) {
    ...
    const responseRate = apps.filter((a: any) => a.status !== 'SUBMITTED').length / Math.max(total, 1);
    const offerRate = apps.filter((a: any) => a.status === 'OFFERED').length / Math.max(total, 1);
  ...
  async getTimeline(userId: string) {
    ...
    return apps.map((a: any) => ({ id: a.id, company: a.company, role: a.role, status: a.status, date: a.createdAt }));
  }
