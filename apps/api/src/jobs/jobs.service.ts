...
export class JobsService {
  ...
  async list(q: any) { return this.prisma.jobListing.findMany({ where: { isActive: true }, take: q.limit || 20, skip: q.skip || 0, orderBy: { postedDate: 'desc' } }); }
  async search(q: any) {
    ...
    return this.prisma.jobListing.findMany({ where, take: 20, orderBy: { postedDate: 'desc' } });
  }
  async findById(id: string) { return this.prisma.jobListing.findUnique({ where: { id } }); }
  async save(userId: string, dto: any) { return this.prisma.jobListing.create({ data: { ...dto, isActive: true } }); } // Simplified for MVP
